import argparse #for arg passing
import collections
import configparser
...
def repo_file(repo,*path, mkdir = False):
    """Same as repo_path , but create dir of path if needed"""

    if repo_dir(repo,*path[:-1],mkdir=mkdir):
        return repo_path(repo,*path)
    
def repo_dir(repo,*path,mkdir=False):
    """same as repo_path, but mkdie *path if absent if mkdir"""
...
def repo_create(path):
    ...
    assert repo_dir(repo,"ref","tags",mkdir=True)
...
def object_write(obj , repo = None):
    ...
    sha = hashlib.sha1(result).hexidigest()
